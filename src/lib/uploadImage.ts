...
/**
 * 图片上传：优先浏览器直传 OSS，图片不经过本服务，省一次上下行带宽、大图更快。
 * Bucket 没配跨域（或直传被网络拦截）时自动回落到 /api/upload 中转，用户无感。
 */
...
async function putDirect(signed: Signed, file: File): Promise<string | null> {
  ...
  const data = await res.json().catch(() => ({}));
  if (!res.ok) throw new Error(data.error ?? "图片登记失败");
...
/** 上传一张图片，返回可访问 URL；失败抛出带原因的 Error */
