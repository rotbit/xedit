 ...
 * 点号开头的条目（.obsidian/.xedit/.trash/.DS_Store…）一概跳过。
 */
...
/** 递归扫库：只有 .md 算文章，但每个非点号目录（哪怕空的）都算一个分类 */
...
export async function moveFile(
  ...
  await copyFile(fh, toDir, toName);
...
async function copyFile(
  ...
  const out = await (await toDir.getFileHandle(name, { create: true })).createWritable();
  try {
